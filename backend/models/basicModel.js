...
let postOrder = async (userID, shopID, order_id, orderStatus, productName, qty, totalPrice) => {
    let sqlCommand = `INSERT INTO orders( buyer_id, seller_id, order_id, order_status, product_name, qty, price) VALUES (${userID}, ${shopID}, ${order_id}, "${orderStatus}", "${productName}", ${qty}, ${totalPrice});`
    let result = await query(sqlCommand);
    return result;
} 
...
let getOrderList = async (orderID) => {
    let sqlCommand = `SELECT id, buyer_id, seller_id, order_id, order_status, order_accept_time, scheduled_time, product_name, qty, price FROM orders WHERE order_id = ${orderID};`
    let result = await query(sqlCommand);
    return result;
} 
